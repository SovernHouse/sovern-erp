...
export const SETTINGS = {
  ...
  aiInternalApprovals: 'Phase 4.15d-1: the AI assistant submits, lists, approves, rejects, and cancels internal approvals from chat. Self-approval is hard-blocked at both submit and decide time (the requester cannot also be the approver). Assigned-only approvals are restricted to the assignee unless overridden by a super_admin. Ask "submit a send_quotation approval for QOT-2026-001 assigned to Maria" or "approve approval ABC-123 with note LGTM".',
  aiProductSpecs: 'Phase 4.15d-2a: the AI assistant manages the ProductSpecification knowledge base from chat. One spec row per product (typed wide-table: 33 named fields covering flooringType, dimensions, AC/fire/slip ratings, surface, wood specifics, installation, packaging, warranty, certifications, origin). Tools: upsert (create/patch), get by id-or-SKU, list with filters, search across string fields, lookup_spec_qa (natural-language Q&A — "what is the AC rating of IL-180x1220-7.5mm?"), archive. The QA tool aliases 71 user-facing field names to the canonical columns.',
  aiLandedCost: 'Phase 4.15b-1: the AI assistant manages LandedCostTemplates + persisted LandedCostCalculations from chat. Templates capture standard freight/insurance/duty/handling/delivery percentages by supplier or origin/destination pair. erp_persist_landed_cost_calculation computes the breakdown AND inserts a row (LCC-YYYYMMDD-NNN ref). When you pass an origin AND a ProductPrice for that (product, origin) has expired, the tool refuses with price_expired and points you at erp_create_product_price. The pure-calc-no-persist calculate_landed_cost (Phase 4.9) remains for quick what-ifs.',
  aiCompliance: 'Phase 4.15d-2b-1: the AI assistant runs compliance checks + duty calculations + HS code lookups from chat. erp_compliance_check applies the rule set (CN→US anti-dumping, any→US CPSC, →EU CE marking, customs always) — stateless, no DB writes. erp_lookup_hs_codes searches the HarmonizedCode catalog by code or description. erp_calculate_duties pulls the rate from HarmonizedCode (with country-specific override support) and computes the duty amount. Plus read-only lookups for ComplianceRecord and CertificateOfOrigin rows. Write tools (record CRUD + HS code creation) come in 4.15d-2b-2.',
  aiContainerLoading: 'Phase 4.15c-1: the AI assistant plans container loads from chat. erp_optimize_container_load is pure-math (no DB write): pass a container_type (20ft / 40ft / 40ft_hc) + list of {product_id, quantity} and get total weight + cube + utilization % + fits/overflow. Flags products missing cubicMeters spec data. erp_create_container_load + update / get / list persist a Container row (status: planning → loading → loaded → in_transit → delivered). Ask "will 500 boxes of IL-180x1220-7.5 fit in a 40ft HC?" or "create a planning container for SO-2026-014 to LA".',
  aiInspection: 'Phase 4.15c-2: the AI assistant runs the full QC inspection lifecycle from chat. State machine: scheduled → in_progress → passed/failed/conditional. Tools: schedule (pre_production / during_production / pre_shipment / loading), start, complete; add + update checkpoint items (refused on finalized inspections); list + get with items/report eager-loaded; generate + get inspection report (auto-derived findings: counts + per-checkpoint breakdown). One-to-one inspection ↔ report. Ask "schedule a pre-shipment inspection for PO-2026-099 at HanHua next Tuesday with Maria as inspector".',
  aiSampleManagement: 'Phase 4.15c-3: the AI assistant manages the full sample request → shipment → feedback flow from chat. Tools: create / approve sample request (auto-sums totalQuantity from products[]); create sample shipment (auto-promotes parent approved/processing → shipped, supports split deliveries); record sample feedback (1-5 overall rating + quality/packaging/delivery sub-axes, auto-escalates when rating ≤ 2); list + get sample request with shipments/feedback/customer eager-loaded. Ask "create a sample request for Acme Imports with 10 sqm of IL-180" or "record 4-star feedback for SR-2026-007".',
  aiLetterOfCredit: 'Phase 4.15b-2: the AI assistant manages Letter of Credit lifecycle from chat. State machine: draft → submitted → approved → presented → paid. SUPER_ADMIN gate on approval (high-stakes financial commitment); self-approval blocked (submitter ≠ approver enforced). Tools: create / submit / approve LC; attach LC document (invoice / B-of-L / packing list / CO / inspection report / insurance / amendment / other); record LC payment with tolerance-checked paid_amount (amount ± tolerance% or amount ± tolerance_amount); list / get LC with documents + supplier + customer eager-loaded. Payment outside tolerance returns a discrepancy error for manual resolution.',
}
