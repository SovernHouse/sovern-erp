...
## Last Updated
2026-05-10 16:55 Taiwan time (thirteenth session, final — Real root cause was the QR's `channel-name=main` but I was publishing to `--branch preview`. Fixed channel routing AND fixed the dead `Picker` import in expenses.tsx that has been latent since `197723f`. Project briefly bumped to SDK 55 but reverted after Alex's screenshot showed his Expo Go is still on SDK 54 ("Not compatible with this version of Expo Go" on every SDK 55 update). Final state: commit `48f12cf` on main, project on SDK 54 + RN 0.81.5 + `@react-native-picker/picker@2.11.1`, expenses.tsx Picker fix preserved. Latest OTA published to main: group `c429dbf3-8c93-422e-bd2e-193cb64f39cb`, iOS update ID `019e111b-9dc2-76f1-8ed2-f2ea5f5078c9`, runtime `1.0.0`. Verified via curl: CDN returns HTTP 200 to `channel-name=main, runtime=1.0.0, ios` query with the matching update ID. Alex deleted Sovern app inside Expo Go and is rescanning the QR at /settings/mobile-app — should now load cleanly. L-039 rewritten with the four-layer diagnostic framework. Future SDK 55 upgrade is deferred until App Store ships SDK 55 Expo Go to Alex's phone.)
